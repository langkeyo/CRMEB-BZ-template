<template>
  <view class="house-rental">
    <!-- 遮罩层 -->
    <view class="mask-layer" v-if="showAreaFilter || showPriceFilter || showSizeFilter || showTypeFilter"
    ...
    <view class="header-section">
      ...
      <view class="search-section">
        ...
        <view class="search-box">
          ...
          <!-- 搜索按钮 -->
          <view class="search-btn" @click="searchHouses">搜索</view>
        </view>
      </view>
    ...
    <view class="filter-dropdown" v-if="showAreaFilter">
      <view class="dropdown-item" v-for="(item, index) in areaOptions" :key="index" @click="selectFilter('area', item)">
        <text>{{ item }}</text>
      </view>
    </view>

    <view class="filter-dropdown" v-if="showPriceFilter">
      <view class="dropdown-item" v-for="(item, index) in priceOptions" :key="index"
    ...
    <view class="filter-dropdown" v-if="showSizeFilter">
      <view class="dropdown-item" v-for="(item, index) in sizeOptions" :key="index" @click="selectFilter('size', item)">
        <text>{{ item }}</text>
      </view>
    </view>

    <view class="filter-dropdown type-filter" v-if="showTypeFilter">
      <view class="dropdown-item" v-for="(item, index) in typeOptions" :key="index" @click="selectFilter('type', item)">
        <text>{{ item }}</text>
      </view>
    </view>
    ...
    <scroll-view scroll-y class="house-list">
      <view class="house-grid">
        <view class="house-card" v-for="(item, index) in houseList" :key="item.id" @click="viewDetail(item.id)">
          <view class="house-img">
            <image :data-index="index" :src="setDomain(item.image)" @error="handleImageError" mode="aspectFill"></image>
          </view>
          <view class="house-info">
            <view class="house-title">{{ item.title }}</view>
            <view class="house-bottom-row">
              <view class="house-address">{{ item.district }}{{ item.business_district ? ' ' + item.business_district :
                '' }}</view>
...
import default_img from '@/static/images/common/empty-image.png'
...
export default {
  ...
  methods: {
    ...
    handleImageError(event) {
      ...
      this.houseList[index].image = default_img
    },
    ...
    // 搜索房屋
    searchHouses() {
      this.queryParams.search = this.keyword.trim()
      this.queryParams.page = 1
      this.houseList = []
      this.finished = false
      this.getHouseList()
