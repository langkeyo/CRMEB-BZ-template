			...
			<view class="order-item" v-for="(item, index) in orderList" :key="index">
				...
				<view class="products-list">
					...
					<view class="product-info"
						v-for="(product, productIndex) in getOrderProducts(item)"
						:key="productIndex"
						@click="goOrderDetails(item.id || item.order_id)">
				...
				<view class="order-actions">
					<view class="action-btn service-btn" @click.stop="applyAfterSales(item)" v-if="item.status === 1 || item.status === 2">申请售后</view>
					<view class="action-btn comment-btn" @click.stop="goComment(item)" v-if="item.status === 2">去评论</view>
		...
		goPay(item) {
			payGroupOrder(item.id || item.order_id).then(res => {
				if (res.status === 200) {
					// 处理支付逻辑
					uni.navigateTo({
						url: `/pages/goods/order_pay_status/index?order_id=${item.id || item.order_id}`
					});
				} else {
					uni.showToast({
						title: res.msg || '支付失败',
