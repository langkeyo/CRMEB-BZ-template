<template>
	<view :style="colorStyle">
		...
		<view class="browse-content" v-if="activeTab === 'browse'">
			<!-- 按日期分组 -->
			<view class="date-group" v-for="(group, groupIndex) in browseProductList" :key="groupIndex">
				<!-- 日期标题 -->
				<view class="date-title">
					<view class="date-dot" :style="{ backgroundColor: '#e93323' }"></view>
					<text style="color: #FE8D00">{{ formatDate(group.visit_time_text) }}</text>
				</view>

				<!-- 商品列表 -->
				<view class="product-list">
					<view class="product-item" v-for="(item, itemIndex) in group.items" :key="itemIndex"
						@click="jump(item)">
						<image class="product-image" :src="item.goods.image"></image>
						<view class="product-info">
							<view class="product-name">{{ item.goods.title }}</view>
							<!-- <view class="product-spec">{{ item.goods.spec || '单份' + item.goods.name + '/40g' }}
							</view> -->
							<view class="product-price">¥ {{ item.goods.max_price }}</view>
						</view>
					</view>
				</view>
			</view>
		...
		<view class='footer acea-row row-between-wrapper'
			v-if="checkbox_show && collectProductList.length && activeTab === 'collect'">
			<view>
	...
	methods: {
		formatDate(date) {
			...
			if (visitDate.isSame(today, 'day')) {
				return '今天'
			} else if (visitDate.isSame(yesterday, 'day')) {
				return '昨天'
			} else if (visitDate.isSame(today, 'week')) {
				// Same week, show day of week
				return visitDate.format('dddd')
			} else {
				// Older dates, show full date
				return visitDate.format('YYYY-MM-DD')
			}
		},
		showRadio() {
				...
				if (res.data.status === 200) {
					...
					let productList = list.map(item => {
						return {
							pid: item.fav_id,
							id: item.id,
							store_name: item.goods?.title || '未知商品',
							price: `${item.goods.min_price}~${item.goods.max_price}`,
							image: this.$util.getImageUrl(item.goods?.image) || '',
							is_show: item.goods?.status === 1, // 1表示正常，0表示已下架
							checked: false
						}
					})
			...
			try {
				...
				if (res.data && res.data.list) {
					...
					res.data.list.forEach(item => {
						const dateKey = dayjs(item.visit_time_text).format('YYYY-MM-DD')
						if (!grouped[dateKey]) {
							grouped[dateKey] = []
						}
						grouped[dateKey].push({ ...item, goods: { ...item.goods, image: this.$util.getImageUrl(item.goods.image) } })
					})
					...
					const sortedGroups = Object.entries(grouped)
						...
						.map(([date, items]) => ({
							dateKey: date,
							visit_time_text: date,
							items: items
						}))

					this.browseProductList = sortedGroups
				}
