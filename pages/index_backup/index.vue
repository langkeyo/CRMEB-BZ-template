		...
		<view v-if="!errorNetwork" :style="colorStyle">
			...
			<view class="fixed z-1000" :style="[appletStyle]" v-if="myApplet">
				<view class="myApplet w-324 h-62 text-center rd-12rpx lh-62rpx fs-24 bg--w111-fff text-w111-303133">
					点击添加到我的小程序<text class="iconfont icon-ic_close2 text--w111-ccc ml-16"
						@click="myApplet = false"></text></view>
			</view>
			<!-- #endif -->
			<!-- 轮播搜索 -->
			<homeComb v-if="showHomeComb" :dataConfig="homeCombData" :belongIndex='belongIndex' @bindSortId="bindSortId"
				:isScrolled="isScrolled" @storeTap="storeTap"></homeComb>
			<!-- 顶部搜索框 -->
			<headerSerch v-if="isHeaderSerch" :dataConfig="headerSerchCombData" :belongIndex='belongIndex'
				@storeTap="storeTap"></headerSerch>
							...
							<scroll-view scroll-x="true" class="white-nowrap vertical-middle w-full"
								...
								<view class="inline-block mr-24" v-for="(item, index) in sortList.children"
									...
									<view class="flex-col flex-center" @tap="changeSort(item, index)">
										...
										<text class="fs-24 pt-14" :class="{ 'font-num': curSort == index }">{{
											item.cate_name }}</text>
									</view>
								</view>
							</scroll-view>
						</view>
						<waterfallsFlow ref="waterfallsFlow" :wfList="goodList" :goDetail="'goDetail'"
							@itemTap="goDetail"></waterfallsFlow>
