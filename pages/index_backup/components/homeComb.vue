		...
		<view class="my-main">
			<view class="mp-header" id="home" :style="[mpHeaderStyle]">
				...
				<view class="serch-box">
					<view class="serch-wrapper flex" :class="special ? 'on' : ''">
						<view class="title skeleton-rect" :style="[titleStyle]" v-if="searchBox == 0">{{ titleConfig }}
						</view>
						<view class="logo skeleton-rect" v-if="searchBox == 1">
							<image :src="logoUpImg" mode="heightFix"></image>
						</view>
						<navigator v-if="hotWords.length"
						...
						<navigator v-else url="/pages/goods/goods_search/index" hover-class="none"
							class="skeleton-rect input" :style="[inputStyle]">
			...
			<view v-if="isCategory" class="category_count">
				...
				<view class="cate_count grid-column-4 grid-gap-16rpx mt-32">
					<view class="category_item" :style="[index === tabClick ? classColor : '']"
						@click="changeTab(item, index)" v-for="(item, index) in tabListConfig" :key="index"
						:id="'ids' + index">
						{{ item.text.val }}
					</view>
				</view>
		...
		<view class="swiperBg" :style="{ paddingBottom: isMenu ? '20rpx' : '20rpx' }">
			<view class="swiper page_swiper" v-if="imgUrls.length">
				<swiper :autoplay="true" :circular="circular" :interval="intervals" :duration="duration"
					:current="swiperCur" :previous-margin="swiperMargin" :next-margin="swiperMargin"
					@change="swiperChange">
					<block v-for="(item, index) in imgUrls" :key="index">
						<swiper-item :class="{ active: index == swiperCur, scalex: isScale }">
							<view @click="goDetail(item)" class="slide-navigator acea-row row-between-wrapper">
								<image :src="item.img" :style="[imageStyle]" mode="aspectFill" class="slide-image">
								</image>
							</view>
						</swiper-item>
					</block>
				...
				<view class="dots acea-row" :class="{
					...
					<view v-else class="acea-row" :class="{
						...
						<view v-for="(item, index) in imgUrls" :key="index" class="dot"
							:class="{ active: index == swiperCur }"
							:style="[dotBgColor, index == swiperCur ? dotColor : '']"></view>
					</view>
				</view>
			</view>
		</view>
